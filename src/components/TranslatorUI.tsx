...
// Create a file at src/icons/index.tsx that re-exports the icons you want to use
// Example:
// export { Globe2, ChevronDown, X, Copy, Trash2, Loader2, Sun, Moon } from "lucide-react";
...
export default function TranslatorUI(): JSX.Element {
  ...
  const toggleLanguage = (lang: Language) => setSelected((prev) => prev.includes(lang) ? prev.filter((l) => l !== lang) : [...prev, lang]);
  ...
  const handleTranslate = async () => {
    ...
    const fake: ResultsMap = {};
    selected.forEach((lang) => { fake[lang] = input || ""; });
    setResults(fake);
  ...
  return (
    <main className={`min-h-screen ${theme.bgMain} flex items-center justify-center p-6 relative`}>
      <div className="absolute top-4 right-4 z-30">
        <button onClick={() => setLight(!light)} className={`inline-flex items-center gap-2 px-3 py-2 rounded-2xl border ${theme.soft} ${theme.softHover}`}>
      ...
      <div className="w-full max-w-6xl grid gap-6 md:grid-cols-[1.1fr_1.3fr]">
        <section className={`${theme.card} rounded-2xl shadow-2xl p-5 md:p-6`}>
          <header className="flex items-center gap-3 mb-4">
            <div className={`p-2 rounded-xl ${theme.soft}`}><Globe2 size={18} /></div>
          ...
          <div className="relative">
            <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Escribe o pega tu texto aquí…" className={`w-full h-48 md:h-64 resize-none rounded-2xl ${theme.soft} outline-none ${theme.ring} p-4`} />
            <div className={`absolute bottom-3 right-3 text-xs ${theme.subtextMuted}`}>{charCount} caracteres</div>
          </div>

          <div className="mt-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            <div className="relative" ref={menuRef}>
              <button onClick={() => setOpen(!open)} className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl border ${theme.soft} ${theme.softHover}`}>
                <ChevronDown size={16} />
              ...
              {open && (
                <div className={`absolute z-20 mt-2 w-64 rounded-xl ${theme.card} shadow-xl p-2 max-h-72 overflow-auto`}>
                  {LANGUAGES.map((lang) => {
                    ...
                    return (
                      <button key={lang} onClick={() => toggleLanguage(lang)} className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center justify-between ${active ? (light ? "bg-black/5" : "bg-white/5") : ""} ${theme.softHover}`}>
                        <span>{lang}</span>
            ...
            <div className="flex gap-2">
              <button onClick={clearAll} className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl border ${theme.soft} ${theme.softHover}`}>
                <Trash2 size={16} /> Limpiar
              </button>
              <button onClick={handleTranslate} disabled={!input || !selected.length || isTranslating} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {selected.map((lang) => (
                <span key={lang} className={`inline-flex items-center gap-2 px-2.5 py-1.5 rounded-full text-xs ${theme.soft} border`}>
                  {lang}
                  <button onClick={() => toggleLanguage(lang)} className="hover:opacity-80">
                    <X size={14} />
                  </button>
                </span>
              ))}
        ...
        <section className={`${theme.card} rounded-2xl shadow-2xl p-5 md:p-6`}>
          ...
          {selected.length === 0 ? (
            ...
            <div className="grid gap-3 md:gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
              {selected.map((lang) => (
                <ResultCard key={lang} language={lang} text={results[lang] ?? ""} onCopy={() => copyText(results[lang] ?? "")} light={light} />
              ))}
            </div>
          )}
        </section>
  ...
  return (
    <div className={`rounded-2xl border p-3 md:p-4 flex flex-col min-h-[160px] ${light ? "bg-black/3 border-black/10" : "bg-white/3 border-white/10 backdrop-blur-sm"}`}>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={onCopy} className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg border ${light ? "bg-black/5 border-black/10 hover:bg-black/10" : "bg-white/5 border-white/10 hover:bg-white/10"}`}>
          <Copy size={14} /> Copiar
        </button>
      </div>
      <div className="flex-1">
        {text ? <p className={`text-sm whitespace-pre-wrap ${light ? "text-black/85" : "text-white/85"}`}>{text}</p> : <p className={`text-sm italic ${light ? "text-black/40" : "text-white/40"}`}>Presiona "Traducir"…</p>}
      </div>
    </div>
  );
}
